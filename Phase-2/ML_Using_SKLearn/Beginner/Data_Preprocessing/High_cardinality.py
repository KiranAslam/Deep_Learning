...
#Target Encoding/problem (Target leakage)
...
X_train = X_train.drop(columns=['arrival_date', 'arrival_date_month'])
X_test = X_test.drop(columns=['arrival_date', 'arrival_date_month'])

print(X_train.head())
train_prt_mean = X_train[X_train['country'] == 'PRT'].iloc[0] 
print(f"Train Encoded Value: {train_prt_mean}")


numeric_cols = X_train.select_dtypes(include=[np.number]).columns
X_train_final = X_train_encoded[numeric_cols]
X_test_final = X_test_encoded[numeric_cols] 
